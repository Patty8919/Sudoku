...
public class SudokuPuzzle {
    ...
    public SudokuPuzzle(int rows, int columns, int boxWitdh, int boxHeight, String[] validvalues) {
        this.ROWS = rows;
    ...
    public boolean numInBox(int row, int col, String value) {
        if (inRange(row, col)) {
            ...
            int boxCol = col / this.BOXWITDH;
            int startingRow = boxRow * this.BOXHEIGHT;
            int startingCol = boxCol * this.BOXWITDH;
            for (int r = startingRow; r <= (startingRow + this.BOXHEIGHT) - 1; r++) {
                for (int c = startingCol; c <= (startingCol + this.BOXWITDH) - 1; c++) {
